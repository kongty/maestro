   ...
   public:
    PyConfigurationV2(std::string hw_file_name, std::string dfsl_file_name)
        : ConfigurationV2(
              ...
              std::make_shared<std::vector<int>>(std::vector<int>()),
              std::make_shared<std::vector<bool>>(std::vector<bool>()), 7, 1,
              INT_MAX, INT_MAX, INT_MAX, 70000) {}
    PyConfigurationV2(std::vector<int> noc_bw, std::vector<int> noc_latency,
        ...
        : ConfigurationV2("", "", std::make_shared<std::vector<int>>(noc_bw),
                          ...
                          std::make_shared<std::vector<bool>>(noc_multcast),
                          num_pes, simd_width, top_noc_bw, l1_sram_byte_size,
                          l2_sram_byte_size, offchip_bw) {}
};

void init_API_configuration(py::module &m) {
    py::class_<PyConfigurationV2, std::shared_ptr<PyConfigurationV2>>(
        m, "Configuration")
        .def(py::init<std::string, std::string>())
        .def(py::init<std::vector<int>, std::vector<int>, std::vector<bool>,
                      int, int, int, int, int, int>());
}

class PyAPIV2 : public APIV2 {
   public:
    PyAPIV2(PyConfigurationV2 &config)
        : APIV2(std::make_shared<ConfigurationV2>(config)) {}
    PyAPIV2(PyConfigurationV2 &config, DFA::Layer &layer)
        : APIV2(std::make_shared<ConfigurationV2>(config),
                std::make_shared<DFA::Layer>(layer)) {}
};
